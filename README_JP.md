...
> [!IMPORTANT]
> - 2026年5月23日12:34頃～2026年5月23日22:56頃：git関連の作業ミスによりリポジトリの `Images` と `Model` フォルダが欠損した状態になっておりました。この間に新規インストールしようとした人はそれが原因で失敗したと思われます。
> - ご迷惑をおかけして申し訳ありません。
> - `Images` と `Model` フォルダは `.gitigore` 対象のため、インストール済みで `Update.bat` や `git pull` で更新した人には問題は起きてないはずです。
...
> [!IMPORTANT]
> - 2026年5月23日12:34頃：上記のリンクとファイルの改行コードが誤っていたため修正しました
...
> - Forge Neo の起動時に `aria2.exe` のWindowsファイアウォールアクセス許可ポップアップが出ます。これは Forge Neo の拡張機能 Civitai Browser に組み込まれた [aria2](https://aria2.github.io/) というダウンローダーのためのものです。許可しない場合は Civitai Browser の動作に支障が出る可能性がありますが、許可するかどうかはご自身で判断してください。逆にこのポップアップで許可せず後から許可したくなった場合は、Windows Defenter ファイアウォールの設定で、該当する aria2 への通信を許可してください。
...
本作について分からないこと・不具合・要望がありましたら [Issues](https://github.com/hirorohi03/EasyForgeNeo/issues) にてお知らせください。GitHubに慣れてない人は [X](https://x.com/hirorohi003) や [note](https://note.com/hirorohi03) 関連記事のコメント欄でも結構です。
