...
> [!TIP]
> - Forge Neo と reForge は同時に起動可能です。Forge Neoは初期設定のポート番号 `7860` 、reForgeはポート番号 `7870` で起動します。
> - Forge Neo の起動時に `aria2.exe` へのWindowsファイアウォールへのアクセスを許可するか尋ねるポップアップが出ます。これは Forge Neo の拡張機能 Civitai Browser に組み込まれた [aria2](https://aria2.github.io/) というダウンローダーのためのものです。EasyForgeNeoでもCivitiaからモデル類をダウンロードするために使用していますが、Civitai Browser 組み込みの aria2 への権限付与はご自身で判断してください。逆にこのポップアップで許可せず後から許可したくなった場合は、Windows Defenter ファイアウォールの設定で aria2 への通信を許可してください。
...
- Forge NeoおよびreForgeの各WebUI本体、拡張機能、その他の各ソフトウェアそのものに関するものはそれぞれの配布元にお問い合わせ願います。
